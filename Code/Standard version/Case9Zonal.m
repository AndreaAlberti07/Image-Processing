...
if not(img_l==1) %if image is not gray level
    ...
    img=rgb2gray(img); %convert image to gray level
end
...
selected_option=input('Choose a mask type inserting number :\n1 - Random \n2 - Upper left corner kept\n  - Your choice --> ');
...
[img_adapted, img_r, img_c]=ImgAdapt(img,block_dim); %see function file
img=img_adapted;
...
for i=1:block_dim:img_r
    ...
    for j=1:block_dim:img_c
        block=img((i:i+block_dim-1),(j:j+block_dim-1));
...
ErrAndDisp(Full_img_rec,img,MASK,Eliminated_coeffs);
