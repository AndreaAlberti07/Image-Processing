...
%mask is a 3D array in which are stored the used masks.
    ...
    img=im2double(img);
        ...
        % all elements > mean + 2*sigma cut out
        % all elements < mean + 2*sigma cut out

        img_trans=dct2(img); %do the trasnform
        variance=sqrt(var(img_trans)); %compute variance
        meanval=mean(img_trans(:)); %compute mean
        mask1=img_trans<(meanval+2*variance); %put 1 where condition is respected
        mask1(1,1)=1; %keep the DC coefficient
        mask2=img_trans>(meanval+2*variance); %put 1 where condition is respected
        mask2(1,1)=1; %keep the DC coefficient
